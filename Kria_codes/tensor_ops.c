    ...
    TK_DISPATCH_TYPES(dest->dtype, __func__, {
	   ...
	   */
	   ...
	   /* tiling + data packing */
	    ...
	    for ( int qq = 0 ; qq < src1_q ; qq += tile_q ) {
	        int q_limit = (qq + tile_q > src1_q) ? src1_q : (qq + tile_q);
	        int current_tile_q = q_limit - qq;

	        for ( int rr = 0 ; rr < src2_r ; rr += tile_r ) {
		    int r_limit = (rr + tile_r > src2_r) ? src2_r : (rr + tile_r);
		    int current_tile_r = r_limit - rr;

		/* --- Step 1: Data Packing (Pack B Tile into Stack) --- */
		// 使用 __attribute__((aligned(64))) 幫助 -O3 做 SIMD
		    scalar_t pack_temp[tile_q * tile_r] __attribute__((aligned(64)));

		    for ( int q_pack = 0; q_pack < current_tile_q; ++q_pack ) {
		        // 注意：這裡是 memcpy 不是 memset，且位址要用 &
		        memcpy(&pack_temp[q_pack * tile_r],
			       &src2_ptr[(qq + q_pack) * src2_r + rr],
			       current_tile_r * sizeof(scalar_t));
		    }

		/* --- Step 2: Compute Core (Multi-threaded) --- */
		// #pragma omp parallel for
		    for (int p = 0; p < src1_p; ++p) {
		        scalar_t* d_ptr = &dest_ptr[p * src2_r + rr];
		        const scalar_t* s1_ptr_base = &src1_ptr[p * src1_q + qq];

		        for (int k = 0; k < current_tile_q; ++k) {
			    scalar_t a_val = s1_ptr_base[k];
			    const scalar_t* b_pack_ptr = &pack_temp[k * tile_r];

			    // 這個最內層迴圈會被 -O3 轉化為 NEON SIMD 指令
			    for (int j = 0; j < current_tile_r; ++j) {
			        d_ptr[j] += a_val * b_pack_ptr[j];
			    }
		        }
		    }
	    }
